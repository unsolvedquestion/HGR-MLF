...
from runtime_utils.metrics import safe_auc
...
class Config(object):
    def __init__(self, args):
        ...
        self.raw_data_dir = getattr(args, 'raw_data_dir', './data')
        self.use_bag = False
        self.use_gpu = True
        ...
        self.alpha = 1.0
        self.enable_relation_denoising = True
    ...
    def init_loss_tensor(self):
        return Variable(torch.Tensor([0]).to(device), requires_grad=True)

    def get_relation_weights(self, predict_re):
        if not self.enable_relation_denoising:
            return torch.ones(predict_re.size(0), predict_re.size(1), device=predict_re.device)
        entropy = -(
            predict_re * torch.log(predict_re + 1e-6)
            + (1 - predict_re) * torch.log(1 - predict_re + 1e-6)
        ).mean(dim=-1)
    ...
    def gen_train_facts_anno(self):
        data_file_name = os.path.join(self.raw_data_dir, 'train_annotated.json')
    ...
    def gen_train_facts_distant(self):
        data_file_name = os.path.join(self.raw_data_dir, 'train_distant.json')
        fact_in_train = set([])  # 创建一个无序不重复数据集合
        ori_data = json.load(open(data_file_name))
        for data in ori_data:
            vertexSet = data['vertexSet']
    ...
    def train(self, model_pattern, model_name):
        ...
        for epoch in range(self.max_epoch):
            ...
            total_loss = self.init_loss_tensor()
            all_loss = 0.0
            logging('-' * 79)
            for i_data in tqdm(range(len(self.train_file))):  # 一个batch的数据
                ...
                document_ner = data['document_ner'].to(device)
                document_pos = data['document_pos'].to(device)
                adj_matrix = data['adj_matrix'].to(device)
                sen_matrix = data['sen_matrix'].to(device)
                pos_matrix_h = data['pos_matrix_h'].to(device)
                pos_matrix_t = data['pos_matrix_t'].to(device)
                node_pos = data['node_pos'].to(device)  # 节点特征 最重要
                node_type = data['node_type'].to(device)
                label_matrix = data['label_matrix'].to(device)
                node_relative_pos = data['node_relative_pos'].to(device)
                ...
                relation_weights = self.get_relation_weights(predict_re)
                temp_loss = self.init_loss_tensor()
                node_num = label_matrix.size(0)
                for h_i in range(node_num):
                    for t_j in range(node_num):
                        if h_i == t_j:
                            continue
                        loss = BCE(predict_re[h_i][t_j], label_matrix[h_i][t_j]) * relation_weights[h_i][t_j]
                ...
                if global_step % self.batch_size == 0:
                    all_loss += float(total_loss.detach().cpu().item())
                ...
                output = self.tensor_to_numpy(predict_re)
                labels = self.tensor_to_numpy(label_matrix)
    ...
    def test(self, model, model_name, output=False, input_theta=-1):
        ...
        for data in tqdm(self.test_file):  # 一个batch的数据
            ...
            predicts = self.tensor_to_numpy(predict_re)
            labels = self.tensor_to_numpy(label_matrix)
